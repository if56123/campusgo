...
public class SecondActivity extends CommonActivity {
	private int FIXEDPRICEPRODUCTS = 0;
	private int AUCTIONPRICEPRODUCTS = 1;
	private int MOREPRODUCTS = 2;
	 ...
	 * ListView数组
	 */
	static final String[] TYPE = { "一口价商品", "拍卖商品", "更多分类", "每日精品", "猜你喜欢", };
	...
	class listViewLitongOnclickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> list, View view, int position,
				long id) {
			if (position == AUCTIONPRICEPRODUCTS) {
				intent.setClass(SecondActivity.this, FixedPriceActivity.class);
				startActivity(intent);
			} else if (position == FIXEDPRICEPRODUCTS) {
				intent.setClass(SecondActivity.this, AuctionPriceActivity.class);
				startActivity(intent);
			} else if (position == MOREPRODUCTS) {
				intent.setClass(SecondActivity.this, MoreProductsActivity.class);
				startActivity(intent);
			} else {
	...
	class ListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				holder = new Holder();
				convertView = inflater.inflate(R.layout.common_listview_text,
						null);
				holder.picture = (ImageView) convertView
						.findViewById(R.id.img_pre);
				holder.text = (TextView) convertView.findViewById(R.id.text);
				holder.navigation = (ImageView) convertView
			...
			if (density < 1.0 || density == 1.5) {
				super.listViewAll
						.setLayoutParams(new RelativeLayout.LayoutParams(
								...
								(int) (list.size() * 48 * density)));
			} else {
				super.listViewAll
						.setLayoutParams(new RelativeLayout.LayoutParams(
								RelativeLayout.LayoutParams.FILL_PARENT,
								(int) (list.size() * 47 * density)));
			}
		}
	}
	...
	private void openQiutDialog() {
		new AlertDialog.Builder(this).setTitle("校际购购物")
				.setMessage("是否退出校际购购物？")
