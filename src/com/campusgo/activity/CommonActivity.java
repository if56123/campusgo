...
import com.campusgo.activity.shopping.ShoppingCarActivity;
import com.campusgo.activity.type.TypeLetaoActivity;
...
public class CommonActivity extends ListActivity implements Runnable {
		...
		new AlertDialog.Builder(this)
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						 ...
						 * finish(); } else { Intent intent = new Intent();
						 * boolean exit = true;
	...
	class ImageViewShooping implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				imageViewShooping
						.setImageResource(R.drawable.menu_shopping_cart_pressed);
			}
			return false;
		}
	...
	class ImageViewMyLetao implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				intent.setClass(getApplicationContext(),
						MyCampusgoInforActivity.class);
				startActivity(intent);
				imageViewMyLetao
						.setImageResource(R.drawable.menu_my_letao_pressed);
			}
			return false;
		}
	}
	...
	// Intent intent = new
	// Intent(Intent.ACTION_QUERY_PACKAGE_RESTART,Uri.fromParts("package",mAppEntry.info.packageName,null));
	// intent.putExtra(Intent.EXTRA_PACKAGES, new
	// String[]{mAppEntry.info.packageName});
	// }
}
