package com.campusgo.activity.mycampusgo;
...
public class LetaoPasswordActivity extends CommonActivity implements
		OnClickListener {
	/**
	 * 旧密码
	 */
	...
	public void onClick(View v) {
		if (oldPasswordEditText.getText().toString() == null
			...
			Toast toast = Toast.makeText(getApplicationContext(),
					"密码格式错误或者原密码错误", Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
		} else if (newPasswordEditText.getText().toString() == null
				|| "".equals(newPasswordEditText.getText().toString())) {
			Toast toast = Toast.makeText(getApplicationContext(),
					"新密码格式错误或者不能为空", Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
		} else if (verifyPasswordEditText.getText().toString()
				.equals(newPasswordEditText.getText().toString()) == false) {
			Toast toast = Toast.makeText(getApplicationContext(),
					"新密码和确认密码不一致！", Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
		} else {
			Toast toast = Toast.makeText(getApplicationContext(), "修改成功！",
					Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
