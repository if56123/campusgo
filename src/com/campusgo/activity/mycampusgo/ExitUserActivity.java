...
import com.campusgo.activity.MyCampusgoInforActivity;
	...
	public void onClick(View v) {
		if (v == loginTextView) {
			...
			if (cellPhoneEditText.getText().toString() == null
				...
				Toast toast = Toast.makeText(this, "用户名不能为空,或者用户名错误",
						Toast.LENGTH_SHORT);
				toast.setGravity(Gravity.CENTER, 0, 0);
				toast.show();
			} else if (passWordEditText.getText().toString() == null
					|| "".equals(passWordEditText.getText().toString())) {
				Toast toast = Toast.makeText(this, "密码不能为空,或者密码错误",
						Toast.LENGTH_SHORT);
				toast.setGravity(Gravity.CENTER, 0, 0);
				toast.show();
			} else {
				startActivity(new Intent(this, MyCampusgoInforActivity.class));
			}
